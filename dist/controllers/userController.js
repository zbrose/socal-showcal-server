    ...
    try {
        ...
        if (!foundUser)
            return res.status(400).json({
                error: "There is no user with that email. Register as a new user or try another email.",
            });
        const matchPasswords = await authService.comparePasswords(password, foundUser.password);
        if (!matchPasswords) {
