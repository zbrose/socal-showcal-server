    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        try {
            ...
            if (err &&
                ...
                console.error(`MongoDB host lookup failed on attempt ${attempt}/${maxAttempts}: ${err.message}`);
                console.error("Possible causes: incorrect MONGO_URI, network/DNS issues, firewall, or VPN blocking access.");
                console.error("Check that MONGO_URI is correct, includes the correct hostnames, and that your machine can resolve them (eg. `nslookup <host>`).");
            }
            else {
                console.error(`MongoDB connection attempt ${attempt} failed:`, err && err.message ? err.message : err);
            }
            if (isLast) {
                console.error("Exceeded maximum MongoDB connection attempts. Giving up.");
                throw err;
            }
